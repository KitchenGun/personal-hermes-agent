...
| Job Registry | 자동화 정의를 YAML로 저장합니다. | `jobs/`, `jobs/README.md`, `docs/02-jobs.md`, `docs/job-registry.md` | 반복 작업을 검토 가능한 산출물로 전환합니다. |
| Queue Intake Guard | 지정 Discord 채널의 사용자 메시지를 Kanban 작업으로만 등록합니다. | `plugins/queue_intake_guard/` | 작업 지시 채널에서는 Hermes 직접 실행과 자동 스레드 생성을 피하고 Codex 감독 큐를 단일 진입점으로 둡니다. |
| Workout Command | 운동기록/인바디 Discord 명령을 Sheets 기록으로 라우팅합니다. | `plugins/workout_command/` | 지정 채널 allowlist와 Google Sheets 설정이 있어야 동작합니다. |
| Risk Controls | 쓰기 작업이 있는 자동화의 승인·중복 방지·fail-closed 규칙을 설명합니다. | `jobs/maintenance/workout_automation_safeguards.yaml`, `docs/job-registry.md` | Calendar write 같은 고위험 자동화를 명시적 확인 토큰 뒤에만 실행하도록 모델링합니다. |
...
## 현재 Orchestrator / Profile 구조

이 저장소의 공개 프로필은 `config/hermes.example.yaml`과 `docs/01-architecture.md`에 근거한 sanitized 운영 구조를 설명합니다. 실제 런타임 값은 포함하지 않고, 공개 가능한 경계와 placeholder만 남깁니다.

| 구성 | 저장소 근거 | 설명 |
| --- | --- | --- |
| Profile | `config/hermes.example.yaml`의 `profile: public-sanitized-hermes` | 공개 공유용으로 정제된 Hermes 운영 프로필 예시입니다. |
| Runtime | `runtime.timezone`, `runtime.default_model`, `runtime.mode` | timezone과 기본 모델은 `${...}` placeholder로 표현하고, mode는 `profile-example`로 제한합니다. |
| Security Policy | `security.publishable`, `security.secrets_policy`, `security.forbidden_paths` | publishable 예시임을 명시하고 `.env`, `memory/`, `logs/`, `sessions/`, `db/` 같은 비공개 상태를 제외합니다. |
| Orchestrator Core | `docs/01-architecture.md`, `diagrams/architecture.mmd` | 외부 입력을 해석하고 planning, tool use, delegation을 조정하는 중심 계층으로 설명합니다. |
| Gateway Boundary | `gateway.discord`, `docs/06-gateway.md` | Discord/API 같은 외부 입력을 내부 명령으로 라우팅하되 실제 token과 channel ID는 placeholder로 둡니다. |
| Job Registry | `job_registry.root: jobs`, `prompts/workflows/add-job-to-repo.md` | 대화 기반 Job 추가/수정 요청을 `jobs/.../*.yaml`로 남기는 운영 레지스트리입니다. |
| Memory Policy | `memory.extraction_prompt`, `memory.write_policy` | memory candidate 추출 흐름은 문서화하되, 쓰기는 사용자 승인 필요 정책으로 표현합니다. |
| Skills / Tools / Delegation | `skills/`, `docs/04-skills.md`, `docs/05-tools.md`, `docs/09-delegation.md` | 반복 절차, 외부 작업 경계, 하위 작업 위임 구조를 분리해 설명합니다. |
| Provider Routing | `config/provider-routing.example.yaml`, `docs/08-provider-routing.md` | 작업 특성에 따른 모델/provider 선택 정책 형태를 예시합니다. |

이 구조는 완성된 MCP 서버/클라이언트 구현이 아니라, MCP-compatible 개념을 고려한 tool boundary와 agent operations profile을 설명하는 reference architecture에 가깝습니다.
...
현재 공개 프로필에는 다음 운영 구조가 반영되어 있습니다.

- VM-local job은 PC 파일 접근 없이 실행되는 구조로 분리합니다.
- PC 파일 작업은 별도 SSH 경계에서만 수행하는 것으로 문서화합니다.
- 운동 일정 자동화처럼 Calendar write가 포함될 수 있는 작업은 `/workout confirm <token>` 또는 `/workout deny <token>` 형태의 결정적 승인 명령을 요구합니다.
- Google Sheets `gid`는 metadata로 A1 range에 해석하고, 누락되면 fail-closed 처리합니다.
- Calendar event는 deterministic event ID와 `hermes_marker`, `spec_hash`를 사용해 idempotent upsert를 전제로 합니다.
- 현재 Hermes CLI 상태는 `docs/12-current-hermes-status.md`에 공개 안전 형태로 기록하고, 매주 토요일 `weekly_hermes_agent_status_update` Job으로 갱신합니다.
